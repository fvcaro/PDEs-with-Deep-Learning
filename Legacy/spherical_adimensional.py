...
def loss_IC(r_ic, t_ic):
    ...
    loss_ic = (u_ic - A*r_ic*torch.exp(-((r_ic - r_0) / delta)**2))**2 + (u_ic_t - 0.)**2
    return loss_ic
...
# CUDA_VISIBLE_DEVICES=0 python spherical_adimensional.py > log_spherical_adimensional_$(date +%d-%m-%Y_%H.%M.%S).txt 2>&1 &

# CUDA_VISIBLE_DEVICES=1 python spherical_adimensional.py > log_spherical_adimensional_$(date +%d-%m-%Y_%H.%M.%S).txt 2>&1 &